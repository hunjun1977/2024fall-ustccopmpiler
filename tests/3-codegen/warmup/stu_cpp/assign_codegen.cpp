...
void translate_main(CodeGen *codegen) {
    ...
    //两个备份指针占16字节，一个alloca指针8字节，再分配40字节的空间给数组，再创建两个指针需要16字节，三个中间变量需要12，
    ...
    //codegen->append_inst("addi.w $t0,$t0,4");
    ...
    codegen->append_inst("st.w",{"$t1","$fp",std::to_string(offset_map["%op4"])});
    ...
    //codegen->append_inst("ld.d $t2,$t1,0");
